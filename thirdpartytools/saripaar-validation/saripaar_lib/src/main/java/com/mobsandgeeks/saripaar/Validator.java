...
public class Validator {
     ...
     */
    private ValidationListenerAllFailures mValidationListenerAllFailures;
    ...
    /**
     * MSM addition for getting all validation failures, not only first as in default.
     *
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    public interface ValidationListenerAllFailures {
        ...
        /**
         * Called if any rule failed.
         *
         * @param viewRulePairs all failed rules
         */
        public void onValidationFailed(List<ViewRulePair> viewRulePairs);
    }

    /**
     * Interface definition for validation single view.
     *
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    public interface ValidationSingleListener {
        /**
         * Called when all the {@link Rule}s added to this Validator are valid.
         */
        public void onValidationSucceeded();
        ...
        /**
         * Called if any rule assigned to this view failed.
         *
         * @param rule failed rule
         */
        public void onValidationSingleFailed(Rule rule);
    }
    ...
    /**
     * Validate all rules, returns failed rules list to callback {@link #mValidationListenerAllFailures}.
     */
    public synchronized void validateAllFailures() {
        if (mValidationListenerAllFailures == null) {
            throw new IllegalStateException("Set a " + ValidationListenerAllFailures.class.getSimpleName() +
                    " before attempting to validate.");
        }

        List<ViewRulePair> failedViewRulePairs = validateAllRulesAllFailures();
        if (failedViewRulePairs == null) {
            mValidationListenerAllFailures.onValidationSucceeded();
        } else {
            mValidationListenerAllFailures.onValidationFailed(failedViewRulePairs);
        }
    }

    /**
     * Validate only rules connected with specified view.
     *
     * @param view validation view
     *
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    public synchronized void validateSingle(View view, ValidationSingleListener validationSingleListener) {
        List<ViewRulePair> failedViewRulePairs = validateAllRulesAllFailures();

        if (failedViewRulePairs == null) {
            validationSingleListener.onValidationSucceeded();
        } else {

            ViewRulePair searchPair = null;
            for (ViewRulePair pair : failedViewRulePairs) {
                if (pair.view == view) {
                    searchPair = pair;
                    break;
                }
            ...
            if (searchPair == null) {
                ...
                validationSingleListener.onValidationSingleFailed(searchPair.rule);
            }
        }
    }
    ...
    /**
     * All failures validation listener.
     *
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    public ValidationListenerAllFailures getValidationListenerAllFailures() {
        return mValidationListenerAllFailures;
    }
     ...
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    public void setValidationListenerAllFailures(final ValidationListenerAllFailures validationListenerAllFailures) {
        mValidationListenerAllFailures = validationListenerAllFailures;
    }
     ...
     * @author Ivan_Kobzarev on 5/26/2014.
     */
    private List<ViewRulePair> validateAllRulesAllFailures() {
        if (!mAnnotationsProcessed) {
            createRulesFromAnnotations(getSaripaarAnnotatedFields());
            mAnnotationsProcessed = true;
        }
        ...
        for (ViewRulePair pair : mViewsAndRules) {
            ...
            if (!pair.rule.isValid(pair.view)) {
                if (ret == null) {
                    ret = new ArrayList<ViewRulePair>();
                }
                ret.add(pair);
            }
        }
    ...
    public static class ViewRulePair {
        public View view;
        public Rule rule;
